...
type ApiV1AccountAccountidUserUseridUserauthPostRequest struct {
	ctx context.Context
	ApiService *VoIPUserAPIService
	accountid string
	userid string
	user *ServiceVOIPImpersonateUser
}

// Payload for impersonate a user
func (r ApiV1AccountAccountidUserUseridUserauthPostRequest) User(user ServiceVOIPImpersonateUser) ApiV1AccountAccountidUserUseridUserauthPostRequest {
	r.user = &user
	return r
}

func (r ApiV1AccountAccountidUserUseridUserauthPostRequest) Execute() (*ServiceDocsImpersonateUserGetSingle, *http.Response, error) {
	return r.ApiService.V1AccountAccountidUserUseridUserauthPostExecute(r)
}
...
Impersonate as another user if you have access to admin.
 ...
 @param accountid Account ID, 32 alpha numeric
 @param userid User ID, 32 alpha numeric
 @return ApiV1AccountAccountidUserUseridUserauthPostRequest
*/
func (a *VoIPUserAPIService) V1AccountAccountidUserUseridUserauthPost(ctx context.Context, accountid string, userid string) ApiV1AccountAccountidUserUseridUserauthPostRequest {
	return ApiV1AccountAccountidUserUseridUserauthPostRequest{
		ApiService: a,
		ctx: ctx,
		accountid: accountid,
...
func (a *VoIPUserAPIService) V1AccountAccountidUserUseridUserauthPostExecute(r ApiV1AccountAccountidUserUseridUserauthPostRequest) (*ServiceDocsImpersonateUserGetSingle, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ServiceDocsImpersonateUserGetSingle
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "VoIPUserAPIService.V1AccountAccountidUserUseridUserauthPost")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/account/{accountid}/user/{userid}/userauth"
	localVarPath = strings.Replace(localVarPath, "{"+"accountid"+"}", url.PathEscape(parameterValueToString(r.accountid, "accountid")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"userid"+"}", url.PathEscape(parameterValueToString(r.userid, "userid")), -1)
